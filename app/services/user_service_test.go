...
func TestUserService_UpdateUserPassword(t *testing.T) {
	...
	t.Run("internal server error on GetUser", func(t *testing.T) {
	...
	t.Run("password same as current value", func(t *testing.T) {
	...
	t.Run("error on GenerateHashedPassword", func(t *testing.T) {
		authMock.EXPECT().IsPasswordsMatch(user.PasswordHash, password).Return(false).Times(1)
